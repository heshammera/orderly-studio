...
export default function ServicesPage() {
  ...
  return (
    <main
      dir={isAr ? "rtl" : "ltr"}
      className={`min-h-screen bg-[#07070A] text-white pt-32 pb-24 ${
    ...
    >
      ...
      <Header
        ...
        onOpenProjectBuilder={() => setSelectedDiscipline("uiux")}
      />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        {/* 6 Disciplines Clean Cards Grid */}
          ...
          {disciplineList.map((id) => {
            ...
            return (
              <div
                ...
                className="group p-8 rounded-3xl bg-soft-black border border-white/10 hover:border-white/25 transition-all duration-300 flex flex-col justify-between cursor-pointer shadow-xl relative overflow-hidden"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-mono font-bold uppercase tracking-wider border ${item.badgeBg} ${item.badgeBorder} ${item.badgeText}`}
                    >
                      {isAr ? item.nameAr.split(" ")[0] : item.id.toUpperCase()}
                    </span>
        ...
        <div className="p-10 sm:p-14 rounded-3xl bg-gradient-to-br from-white/[0.05] via-soft-black to-black border border-white/15 text-center flex flex-col items-center shadow-2xl">
          ...
          <p className="text-neutral-cool text-sm max-w-lg mb-8 leading-relaxed">
            {isAr
              ? "نحن متخصصون في دمج الهندسة والتصميم والتسويق في آن واحد لتسليم منتج رقمي متكامل يقود السوق."
              : "We seamlessly unite engineering, brand design, and marketing strategy under one cohesive studio roof."}
          </p>
          <button
